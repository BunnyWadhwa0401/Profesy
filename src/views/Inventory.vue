...
let inventory = ref(localStorage.getItem("Inventory"))
inventory.value = JSON.parse(inventory.value.replace(/'/g, '"'));
...
<template>
  <main class="Inventory-page">
    <Header title="Material Status" />
    <div class="fadeIn grid grid-cols-3 w-full h-[14rem] gap-4 justify-between">
      <section v-for="item in inventory" :key="item">
        <inventory-status :item="item[0]" :total="parseInt(item[1])" :status="parseInt(item[2])" />
      </section>
    </div>
